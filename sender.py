...
def send_encrypted_image(image_data, key):
    HOST = '127.0.0.1'
    PORT = 8910
    
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.connect((HOST, PORT))
        serialized_data = pickle.dumps((image_data, key))
        s.sendall(serialized_data)
        print("Şifrelenmiş resim gönderildi.")
...
def on_button_click():
    ...
    if file_path:
        ...
        send_encrypted_image(encrypted_image_data, key)
        print("Şifrelenmiş veri:", encrypted_image_data)

# Buton oluştur
root = Tk()
root.title("Resmi Şifreli Gönder")
window_width = 400
window_height = 300
